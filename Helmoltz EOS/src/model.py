...
def model(Temperature, Pressure):
    ...
    for T in Temperature:
        ...
        for P in Pressure:
            ...
            # initialization
            Ar_V       = { 'CH4':0, 'CO2':0, 'H2S':0, 'H2O':0 } # --
            entropy_V  = { 'CH4':0, 'CO2':0, 'H2S':0, 'H2O':0 } # J/mol/K
            enthalpy_V = { 'CH4':0, 'CO2':0, 'H2S':0, 'H2O':0 } # J/mol/K
            cp_V       = { 'CH4':0, 'CO2':0, 'H2S':0, 'H2O':0 } # J/mol/K
            cv_V       = { 'CH4':0, 'CO2':0, 'H2S':0, 'H2O':0 } # J/mol/K
            cubicExp_V = { 'CH4':0, 'CO2':0, 'H2S':0, 'H2O':0 } # --
            fuga_V     = { 'CH4':0, 'CO2':0, 'H2S':0, 'H2O':0 } # --
            fuga_m     = { 'CH4':0, 'CO2':0, 'H2S':0, 'H2O':0 } # --
            Ar_L       = { 'CH4':0, 'CO2':0, 'H2S':0, 'H2O':0 } # --
            entropy_L  = { 'CH4':0, 'CO2':0, 'H2S':0, 'H2O':0 } # J/mol/K
            enthalpy_L = { 'CH4':0, 'CO2':0, 'H2S':0, 'H2O':0 } # J/mol/K
            cp_L       = { 'CH4':0, 'CO2':0, 'H2S':0, 'H2O':0 } # J/mol/K
            cv_L       = { 'CH4':0, 'CO2':0, 'H2S':0, 'H2O':0 } # J/mol/K
            cubicExp_L = { 'CH4':0, 'CO2':0, 'H2S':0, 'H2O':0 } # --
            fuga_L     = { 'CH4':0, 'CO2':0, 'H2S':0, 'H2O':0 } # --
            Psat       = { 'CH4':0, 'CO2':0, 'H2S':0, 'H2O':0 } # Pa
            H          = { 'CH4':0, 'CO2':0, 'H2S':0, 'H2O':0 } # Pa
            dens_L     = { 'CH4':0, 'CO2':0, 'H2S':0, 'H2O':0 } # kg * m^(-3)
            dens_V     = { 'CH4':0, 'CO2':0, 'H2S':0, 'H2O':0 } # kg * m^(-3)
            x          = { 'CH4':0, 'CO2':0, 'H2S':0, 'H2O':0 } # mol/mol
            y          = { 'CH4':0, 'CO2':0, 'H2S':0, 'H2O':0 } # mol/mol
            Lx         = { 'CH4':0, 'CO2':0, 'H2S':0, 'H2O':0 } # kmol/d
            Vy         = { 'CH4':0, 'CO2':0, 'H2S':0, 'H2O':0 } # kmol/d

            for specie in species:
                
                if specie != 'H2O':
                    H[specie] = Henry(kH[specie], T)*1e3
                else:
                    pass
                
                Psat[specie]       = Antoine(Ant[specie], T)*101325
            ...
            k = {
                'CH4': H['CH4']   *fuga_L['CH4'] / (P*fuga_m['CH4']),
                'CO2': H['CO2']   *fuga_L['CO2'] / (P*fuga_m['CO2']),
                'H2S': Psat['H2S']*fuga_L['H2S'] / (P*fuga_m['H2S']),
                'H2O': Psat['H2O']*fuga_L['H2O'] / (P*fuga_m['H2O'])  
                } # Pa/Pa
            ...
            with open(file, 'w') as res:
                ...
                for specie in species:
                    ...
                    res.write(f'\nFugacity coeffcient________________: {"{:.5f}".format(fuga_L[specie])} [--]')
                    res.write('\n')
    ...
    # change by hand every time
    lim_min = min(Vap_TP[7])/1.02; lim_max = max(Vap_TP[0])
    ...
    # change by hand every time
    lim_min = min(Liq_TP[0])-5; lim_max = max(Liq_TP[7])+5
    ...
    # change by hand every time
    lim_min = min(x_CH4[7]); lim_max = max(x_CH4[0])
    ...
    # change by hand every time
    lim_min = min(x_CO2[7]); lim_max = max(x_CO2[0])
    ...
    # change by hand every time
    lim_min = min(x_H2S[7]); lim_max = max(x_H2S[0])
    ...
    # change by hand every time
    lim_min = min(y_CH4[7]); lim_max = max(y_CH4[0])
    ...
    # change by hand every time
    lim_min = min(y_CO2[7]); lim_max = max(y_CO2[0])
    ...
    # change by hand every time
    lim_min = min(y_H2S[7]); lim_max = max(y_H2S[0])
    ...
    # change by hand every time
    lim_min = min(y_H2O[7]); lim_max = max(y_H2O[0])
    ...
    return print('simulation successfull')