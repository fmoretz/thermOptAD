...
def mole_day(lks: list, hk: str, Temp_degC, Pres_atm, Height, Diameter, showbool=False):
   ...
   # diffusion coefficients for i species in water (m2/s)
    ...
    axes.plot(T, n_03atm[A[0]], marker='o', markersize=ms, linewidth=lw, label=f'P: ${Pres_atm[0]}\,atm$')
    axes.plot(T, n_05atm[A[0]], marker='^', markersize=ms, linewidth=lw, label=f'P: ${Pres_atm[1]}\,atm$')
    axes.plot(T, n_10atm[A[0]], marker='s', markersize=ms, linewidth=lw, label=f'P: ${Pres_atm[2]}\,atm$')
    axes.plot(T, n_15atm[A[0]], marker='D', markersize=ms, linewidth=lw, label=f'P: ${Pres_atm[3]}\,atm$')
    axes.ticklabel_format(axis='y', style='sci', scilimits=[-2, 0])
    axes.set_ylabel(filename)
    axes.grid(color='k', alpha=0.2, linestyle='dashed', linewidth=0.5)
    axes.set_title('CH$_4$')
    axes.xaxis.set_ticks(np.linspace(Temp_degC[0], Temp_degC[-1], len(Temp_degC)), endpoint=True)
    axes.yaxis.set_ticks(np.linspace(0,0.15, len(Temp_degC)+1), endpoint=True)
    fig.savefig(fname='absolute_mol_flux_CH4.svg', format='svg', dpi=dpi)

    fig, axes = plt.subplots(dpi=dpi)
    filename = 'absolute molar flux [mol/d]'
    axes.set_ylabel(filename)
    axes.plot(T, n_03atm[A[1]], marker='o', markersize=ms, linewidth=lw, label=f'P: ${Pres_atm[0]}\,atm$')
    axes.plot(T, n_05atm[A[1]], marker='^', markersize=ms, linewidth=lw, label=f'P: ${Pres_atm[1]}\,atm$')
    axes.plot(T, n_10atm[A[1]], marker='s', markersize=ms, linewidth=lw, label=f'P: ${Pres_atm[2]}\,atm$')
    axes.plot(T, n_15atm[A[1]], marker='D', markersize=ms, linewidth=lw, label=f'P: ${Pres_atm[3]}\,atm$')
    axes.ticklabel_format(axis='y', style='sci', scilimits=[-2, 0])
    axes.grid(color='k', alpha=0.2, linestyle='dashed', linewidth=0.5)
    axes.set_title('CO$_2$')
    axes.xaxis.set_ticks(np.linspace(Temp_degC[0], Temp_degC[-1], len(Temp_degC)), endpoint=True)
    axes.yaxis.set_ticks(np.linspace(0.02,0.3, len(Temp_degC)+1), endpoint=True)
    fig.savefig(fname='absolute_mol_flux_CO2.svg', format='svg', dpi=dpi)

    fig, axes = plt.subplots(dpi=dpi)
    filename = 'absolute molar flux [mol/d]'
    axes.set_ylabel(filename)
    axes.plot(T, n_03atm[A[2]],marker='o', markersize=ms, linewidth=lw, label=f'P: ${Pres_atm[0]}\,atm$')
    axes.plot(T, n_05atm[A[2]],marker='^', markersize=ms, linewidth=lw, label=f'P: ${Pres_atm[1]}\,atm$')
    axes.plot(T, n_10atm[A[2]],marker='s', markersize=ms, linewidth=lw, label=f'P: ${Pres_atm[2]}\,atm$')
    axes.plot(T, n_15atm[A[2]],marker='D', markersize=ms, linewidth=lw, label=f'P: ${Pres_atm[3]}\,atm$')
    axes.ticklabel_format(axis='y', style='sci', scilimits=[-2, 0])
    axes.set_title('H$_2$S')
    axes.grid(color='k', alpha=0.2, linestyle='dashed', linewidth=0.5)
    axes.xaxis.set_ticks(np.linspace(Temp_degC[0], Temp_degC[-1], len(Temp_degC)), endpoint=True)
    axes.yaxis.set_ticks(np.linspace(0,0.1, len(Temp_degC)+1), endpoint=True)
    fig.savefig(fname='absolute_mol_flux_H2S.svg', format='svg', dpi=dpi)

    fig, axes = plt.subplots(dpi=dpi)
    filename = 'normalized molar flux [mol/d]'
    axes.set_ylabel(filename)
    axes.plot(T, n_03atm[A[0]]/np.max(n_03atm[A[0]]),marker='o', linewidth=lw, markersize=ms, label=f'P: ${Pres_atm[0]}\,atm$')
    axes.plot(T, n_05atm[A[0]]/np.max(n_05atm[A[0]]),marker='^', linewidth=lw, markersize=ms, label=f'P: ${Pres_atm[1]}\,atm$')
    axes.plot(T, n_10atm[A[0]]/np.max(n_10atm[A[0]]),marker='s', linewidth=lw, markersize=ms, label=f'P: ${Pres_atm[2]}\,atm$')
    axes.plot(T, n_15atm[A[0]]/np.max(n_15atm[A[0]]),marker='D', linewidth=lw, markersize=ms, label=f'P: ${Pres_atm[3]}\,atm$')
    axes.ticklabel_format(axis='y', style='sci', scilimits=[-2, 0])
    axes.grid(color='k', alpha=0.2, linestyle='dashed', linewidth=0.5)
    axes.set_title('CH$_4$')
    axes.set_xlabel('T [°C]');
    axes.xaxis.set_ticks(np.linspace(Temp_degC[0], Temp_degC[-1], len(Temp_degC)), endpoint=True)
    axes.yaxis.set_ticks(np.linspace(0.9,1, len(Temp_degC)+1), endpoint=True)
    fig.savefig(fname='normalized_mol_flux_CH4.svg', format='svg', dpi=dpi)

    fig, axes = plt.subplots(dpi=dpi)
    filename = 'normalized molar flux [mol/d]'
    axes.set_ylabel(filename)
    axes.plot(T, n_03atm[A[1]]/np.max(n_03atm[A[1]]),marker='o', linewidth=lw, markersize=ms, label=f'P: ${Pres_atm[0]}\,atm$')
    axes.plot(T, n_05atm[A[1]]/np.max(n_05atm[A[1]]),marker='^', linewidth=lw, markersize=ms, label=f'P: ${Pres_atm[1]}\,atm$')
    axes.plot(T, n_10atm[A[1]]/np.max(n_10atm[A[1]]),marker='s', linewidth=lw, markersize=ms, label=f'P: ${Pres_atm[2]}\,atm$')
    axes.plot(T, n_15atm[A[1]]/np.max(n_15atm[A[1]]),marker='D', linewidth=lw, markersize=ms, label=f'P: ${Pres_atm[3]}\,atm$')
    axes.ticklabel_format(axis='y', style='sci', scilimits=[-2, 0])
    axes.set_xlabel('T [°C]');
    axes.grid(color='k', alpha=0.2, linestyle='dashed', linewidth=0.5)
    axes.set_title('CO$_2$')
    axes.xaxis.set_ticks(np.linspace(Temp_degC[0], Temp_degC[-1], len(Temp_degC)), endpoint=True)
    axes.yaxis.set_ticks(np.linspace(0.7,1, len(Temp_degC)+1), endpoint=True)
    fig.savefig(fname='normalized_mol_flux_CO2.svg', format='svg', dpi=dpi)

    fig, axes = plt.subplots(dpi=dpi)
    filename = 'normalized molar flux [mol/d]'
    axes.set_ylabel(filename)
    axes.plot(T, n_03atm[A[2]]/np.max(n_03atm[A[2]]),marker='o', linewidth=lw, markersize=ms, label=f'P: ${Pres_atm[0]}\,atm$')
    axes.plot(T, n_05atm[A[2]]/np.max(n_05atm[A[2]]),marker='^', linewidth=lw, markersize=ms, label=f'P: ${Pres_atm[1]}\,atm$')
    axes.plot(T, n_10atm[A[2]]/np.max(n_10atm[A[2]]),marker='s', linewidth=lw, markersize=ms, label=f'P: ${Pres_atm[2]}\,atm$')
    axes.plot(T, n_15atm[A[2]]/np.max(n_15atm[A[2]]),marker='D', linewidth=lw, markersize=ms, label=f'P: ${Pres_atm[3]}\,atm$')
    axes.ticklabel_format(axis='y', style='sci', scilimits=[-2, 0])
    axes.set_xlabel('T [°C]');
    axes.grid(color='k', alpha=0.2, linestyle='dashed', linewidth=0.5)
    axes.set_title('H$_2$S')
    axes.xaxis.set_ticks(np.linspace(Temp_degC[0], Temp_degC[-1], len(Temp_degC)), endpoint=True)
    axes.yaxis.set_ticks(np.linspace(0.9,1, len(Temp_degC)+1), endpoint=True)
    fig.savefig(fname='normalized_mol_flux_H2S.svg', format='svg', dpi=dpi)
