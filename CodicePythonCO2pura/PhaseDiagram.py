...
import SWcorrelationEq as SWcE
...
print("The algorithm are reading the data file")

# Reading data set (data range is: 73.15 K<T<323.15 K and 0.01 MPa<P<10 MPa)
path = 'PureCO2.xlsx'
data = pd.read_excel(path, sheet_name="data", index_col=0)
...
def EquilibriumSolver(eps,k,T):
    ...
    if x < 0: 
	    raise Exception('WARNING: P_eq', x*1.0D-06, 'calculated at temperature', T,'has no physical meaning')
...
def f(x,k,T):
    ...
    if k == 1:
        ...
        retur (R*T*(A_r(1) - A_r(2)) - Mco2*x*((1/d(2)) - (1/d(1))))
    elif k == 3:
        retur g - R*T*A_r(1) - Mco2*(x/d(1))
