...
def phi0(tau,d_r,species):
    ...
    from prop import a0, th0, a, T_c

    B0 = 0.
    for i in range(len(a0[species])):
        B0 = B0 + a0[species][i]*mt.log(1-mp.exp(-tau*th0[species][i])) 

    # Ideal-gas part of the dimensionless Helmholtz energy  
    return (log(d_r) + a[species][0] + a[species][1]*tau + a[species][2]*mt.log(tau) + a[species][3]*(T_c[species]/tau)**1.5 + B0)
...
def phiR(tau,d_r,species):
    ...
    from prop import n1, n2, n3, n4
    from prop import d1, t1, d2, t2, c2, d3, t3, alpha3, beta3, gamma3
    from prop import Ai4, alpha4, beta4, bi4, C4, Di4
    ...
    for i in range(len(n2[species])):
        sum2 = sum2 + n2[species][i]*(d_r**d2[species][i])*(tau**t2[species][i])*mt.exp(-d_r**c2[species][i])

    # Calculation of the third term of the Helmholtz energy's residual part
    sum3 = 0.
    for i in range(len(n3[species])):
        sum3 = sum3 + n3[species][i]*(d_r**d3[species][i])*(tau**t3[species][i])*mt.exp(-alpha3[species][i]*(d_r-1)**2-beta3[species][i]*(tau-gamma3[species][i])**2)

    # Calculation of the fourth term of the Helmholtz energy's residual part
    sum4 = 0.
    for i in range(len(n4[species])):
        delta = (1-tau + Ai4[species][i]*((d_r-1)**2)**(1/0.6))**2 + beta4[species][i]*((d_r-1)**2)**alpha4[species][i]
        sum4 = sum4 + n4[species][i]*(delta**bi4[species][i])*d_r*mt.exp(-(C4[species][i]*(d_r-1)**2)-Di4[species][i]*((tau-1)**2))  
...
def dphi0_t(tau,species):
    ...
    return (a[species][1] + a[species][2]/tau - (1.5*a[species][3]*T_c[species]**1.5)/tau**2.5 + sum1)
...
def dphi0_tt(tau,species):
    ...
    sum1 = 0.
    for i in range(len(a0[species])):
        sum1 = sum1 + a0[species][i]*(th0[species][i]**2)*mt.exp(-tau*th0[species][i])*((1-mt.exp(-tau*th0[species][i]))**(-2))

    # Derivative of dphi0_t with respect to tau
    return (-a[species][2]/(tau**2) - sum1 + 2.5*(1.5*a[species][3]*T_c[species]**1.5)/tau**3.5)
...
def dphir_t(tau,d_r,species):
    ...
    for j in range(len(n2[species])):
        sum2 = sum2 + n2[species][j]*t2[species][j]*(d_r**d2[species][j])*(tau**(t2[species][j] -1))*mt.exp(-d_r**c2[species][j])

    # Derivative of the third term of Helmholtz energy's residual part with respect to tau
    sum3 = 0.
    for z in range(len(n3[species])):
        sum3 = sum3 + n3[species][z]*(d_r**d3[species][z])*(tau**t3[species][z])*mt.exp(-(alpha3[species][z]*diff**2)\
        - beta3[species][z]*((tau-gamma3[species][z])**2))*((t3[species][z]/tau) - 2*beta3[species][z]*(tau-gamma3[species][z]))
...
def dphir_tt(tau,d_r,species):
    ...
    sum1 = 0.
    for i in range(len(n1[species])):
        sum1 = sum1 + n1[species][i]*t1[species][i]*(t1[species][i] - 1)*(d_r**d1[species][i])*(tau**(t1[species][i] - 2))

    # Derivative of the second term of dphir_t with respect to tau
    sum2 = 0.
    for j in range(len(n2[species])):
        sum2 = sum2 + n2[species][j]*t2[species][j]*(t2[species][j]-1)*(d_r**d2[species][j])*(tau**(t2[species][j]-2))*mp.exp(-d_r**c2[species][j])
    ...
    for z in range(len(n3[species])):
        ...
        sum3 = sum3 + n3[species][z]*(d_r**d3[species][z])*(tau**t3[species][z])*mt.exp(-(alpha3[species][z]*diff**2)\
        -beta3[species][z]*eTau**2)*((t3[species][z]/tau-2*beta3[species][z]*eTau)**2-t3[species][z]/(tau**2)-2*beta3[species][z])
    ...
    for h in range(len(n4[species])):
        ...
        delta     = theta**2 + beta4[species][h]*(diff**2)**alpha4[species][h]
        deltab_t  = -2*bi4[species][h]*theta*(delta**(bi4[species][h]-1))
        deltab_tt = 2*bi4[species][h]*(delta**(bi4[species][h]-1))+4*(delta**(bi4[species][h]-2))*bi4[species][h]*(bi4[species][h]-1)*theta**2
        psi       = mt.exp(-(C4[species][h]*diff**2) - Di4[species][h]*((tau - 1)**2))
        psi_t     = -2*Di4[species][h]*(tau - 1)*psi
...
def dphi_r(d_r,tau,species):
    ...
    sum2 = 0.
    for j in range(len(n2[species])):
        sum2 = sum2 + n2[species][j]*mt.exp(-d_r**c2[species][j])*(d_r**(d2[species][j]-1))*(tau**t2[species][j])*(d2[species][j]-c2[species][j]*d_r**c2[species][j])

    # Derivative of the third term of Helmholtz energy's residual part with respect to d_r
    sum3 = 0.
    for z in range(len(n3[species])):
        sum3 = sum3 + n3[species][z]*(d_r**d3[species][z])*(tau**t3[species][z])*mt.exp(-(alpha3[species][z]*diff**2)\
        - beta3[species][z]*((tau-gamma3[species][z])**2))*((d3[species][z]/d_r) - 2*alpha3[species][z]*diff)
    ...
    for h in range(len(n4[species])):
        theta  = 1 - tau + Ai4[species][h]*(diff**2)**(1/0.6)
        delta  = theta**2 + beta4[species][h]*(diff**2)**alpha4[species][h]
        delta1 = diff*((2*Ai4[species][h]*theta/0.3)*(diff**2)**(1/0.6-1) + 2*beta4[species][h]*alpha4[species][h]*(diff**2)**(alpha4[species][h]-1))
...
def d2phi_r(d_r,tau,species):
    ...
    sum1 = 0.
    for i in range(len(n1[species])):
        sum1 = sum1 + n1[species][i]*d1[species][i]*(d1[species][i]-1)*(d_r**(d1[species][i]-2))*(tau**t1[species][i])

    # Derivative of the second term of dphi_r with respect to reduced density
    sum2 = 0.
    for j in range(len(n2[species])): 
       dr   = d_r**c2[species][j]
       sum2 = sum2 + n2[species][j]*mt.exp(-dr)*(d_r**(d2[species][j]-2))*(tau**t2[species][j])*((d2[species][j]-c2[species][j]*dr)*(d2[species][j]-1-c2[species][j]*dr)-dr*c2[species][j]**2)
    ...
    for z in range(len(n3[species])):
        ...
        d3z  = d3[species][z] - 1
        sum3 = sum3 + n3[species][z]*(tau**t3[species][z])*mt.exp(-(alpha3[species][z]*diff**2)-beta3[species][z]*eTau**2)*(-2*alpha3[species][z]*dr2 \
            + 4*(alpha3[species][z]**2)*dr2*diff**2 -4*d3[species][z]*alpha3[species][z]*diff*(dr2/d_r) + d3[species][z]*d3z*d_r**(d3z-1))
    ...
    for h in range(len(n4[species])):
        ...
        delta   = theta**2 + beta4[species][h]*(diff**2)**alpha4[species][h]
        d1_1    = (diff**2)**(1/0.6 - 1)
        delta1  = diff*((2*Ai4[species][h]*theta/0.3)*d1_1 + 2*beta4[species][h]*alpha4[species][h]*(diff**2)**(alpha4[species][h]-1))
        d2_1    = 4*beta4[species][h]*alpha4[species][h]*(alpha4[species][h]-1)*(diff**2)**(alpha4[species][h]-2)
        d2_2    = (4*Ai4[species][h]*theta/0.3)*(1/0.6 - 1)*(diff**2)**(1/0.6-2)
        delta2  = (delta1/diff) + (diff**2)*(d2_1 + (2*(Ai4[species][h]**2)/0.09)*d1_1**2 + d2_2)
        deltab  = bi4[species][h]*(delta**(bi4[species][h]-1)*delta1)
        deltab2 = bi4[species][h]*(delta**(bi4[species][h]-1)*delta2 + (bi4[species][h]-1)*(delta**(bi4[species][h]-2))*delta1**2)
        psi     = mt.exp(-(C4[species][h]*diff**2) - Di4[species][h]*((tau-1)**2))
        psi1    = -2*C4[species][h]*diff*psi
        psi2    = 2*C4[species][h]*psi*(2*C4[species][h]*diff**2 - 1)
        sum4    = sum4 + n4[species][h]*((delta**bi4[species][h])*(2*psi1 + d_r*psi2)+2*deltab*(psi + d_r*psi1)+d_r*psi*deltab2)
...
def dphir_rt(tau,d_r,species):
    ...
    sum1 = 0.
    for i in range(len(n1[species])):
        sum1 = sum1 + n1[species][i]*d1[species][i]*t1[species][i]*(d_r**(d1[species][i]-1))*(tau**(t1[species][i] - 1))

    # Derivative of the second term of dphi_r with respect to tau
    sum2 = 0.
    for j in range(len(n2[species])):
        sum2 = sum2 + n2[species][j]*t2[species][j]*mt.exp(-d_r**c2[species][j])*(d_r**(d2[species][j]-1))*(tau**(t2[species][j]-1))*(d2[species][j]-c2[species][j]*d_r**c2[species][j])
    ...
    for z in range(len(n3[species])):
        eTau = tau-gamma3[species][z]
        sum3  = sum3 + n3[species][z]*(d_r**d3[species][z])*(tau**t3[species][z])*mt.exp(-(alpha3[species][z]*diff**2)\
           -beta3[species][z]*eTau**2)*((d3[species][z]/d_r)-2*alpha3[species][z]*diff)*((t3[species][z]/tau)-2*beta3[species][z]*eTau)
    ...
    for h in range(len(n4[species])):
        ...
        delta_rt = -(delta**(bi4[species][h]-1))*(2*Ai4[species][h]*bi4[species][h]/0.3)*diff*(diff**2)**(1/0.6-1) + delta_t
        psi      = mt.exp(-(C4[species][h]*diff**2) - Di4[species][h]*((tau-1)**2))
        psi1     = -2*C4[species][h]*diff*psi
        psi_t    = -2*Di4[species][h]*(tau -1)*psi
        psi_rt   = 4*Di4[species][h]*C4[species][h]*diff*(tau-1)*psi
        sum4     = sum4 + n4[species][h]*(dbi4*(psi_t + d_r*psi_rt) + d_r*psi_t*deltab + deltab_t*(psi+d_r*psi1)+d_r*psi*delta_rt)
...
def VLProperties(q,T,d,species):
    ...
    from prop import r_c
    from prop import T_c
    ...
    # Molar enthalpy
    enthalpy = R*T*(1+tau*(dphi0_t(tau,species) + dphir_t(tau,d_r,species)) + d_r*dphi_r(d_r,tau,species))
    ...
    # Fugacity coeffcient
    phi_coef = mp.exp(phiR(tau,d_r,species) +d_r*dphi_r(d_r,tau,species)-np.log(1 + d_r*dphi_r(d_r,tau,species)))
...
def density(x0,T,P,species):
    ...
    x = opt.newton(rho, x0, fprime=drho, args=(T,P,species), tol = 1.0E-6, maxiter=100)
...
def fug_mix_helmoltz(specie, T, P):
    ...
    from prop import T_c, P_c, w, Ant
